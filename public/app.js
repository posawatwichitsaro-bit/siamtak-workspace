...
function login(){
  app.innerHTML=`<main class="min-h-screen relative overflow-hidden bg-[#f5f7fb] grid place-items-center p-6">
    <div class="absolute -top-32 -left-32 w-96 h-96 bg-indigo-200/40 rounded-full blur-3xl"></div>
    <div class="absolute -bottom-40 -right-20 w-[30rem] h-[30rem] bg-sky-200/40 rounded-full blur-3xl"></div>
    <section class="relative w-full max-w-md bg-white/90 backdrop-blur-xl border border-white rounded-[2rem] shadow-[0_25px_80px_-25px_rgba(15,23,42,.25)] p-8 md:p-10">
      <div class="flex items-center gap-4 mb-9">
        <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-slate-950 to-slate-700 text-white grid place-items-center text-xl font-black shadow-lg">S</div>
      ...
      <form id="login" class="space-y-4">
        ...
        <button class="w-full bg-slate-950 hover:bg-slate-800 text-white rounded-xl py-3.5 font-semibold shadow-lg shadow-slate-950/15 transition">เข้าสู่ Workspace</button>
      </form>
...
function shell(){
  app.innerHTML=`<header class="h-[72px] tech-stone-header backdrop-blur-xl border-b border-slate-300 flex items-center justify-between px-4 md:px-8 sticky top-0 z-40 text-slate-800">
    <div class="flex items-center gap-3">
      <button id="hamb" class="w-10 h-10 rounded-xl hover:bg-slate-100 text-lg transition border border-transparent hover:border-slate-200" aria-label="menu">☰</button>
      <button onclick="hub()" class="font-black tracking-tight text-xl">Siamtak Workspace <span class="text-[10px] ml-2 text-slate-500 tracking-[.25em]">SYSTEM HUB</span></button>
    </div>
    <div class="flex gap-2 items-center text-sm"><span class="hidden sm:block text-slate-500">${esc(me.name)}</span><span class="px-3 py-1.5 bg-slate-100 border border-slate-200 rounded-full font-medium text-slate-600">${esc(me.role)}</span></div>
  </header>
  <aside id="drawer" class="fixed top-[72px] bottom-0 left-0 z-50 w-72 bg-white border-r border-slate-200 p-4 -translate-x-full transition-transform shadow-2xl text-white">
    <div class="px-3 py-3 mb-3 text-xs font-bold tracking-[.25em] text-slate-400">WORKSPACE</div>
    <nav class="space-y-1">
  ...
  <div id="page"></div>
  <style>
body{background:#dfe2e5;color:#1e293b}
.stone-hero{
  background-color:#e5e2dc;
  background-image:
    radial-gradient(ellipse at 18% 20%,rgba(255,255,255,.72) 0 8%,transparent 30%),
    radial-gradient(ellipse at 78% 68%,rgba(112,101,88,.12) 0 5%,transparent 28%),
    linear-gradient(115deg,transparent 0 28%,rgba(255,255,255,.35) 29%,transparent 32%),
    linear-gradient(70deg,transparent 0 58%,rgba(118,105,91,.09) 59%,transparent 62%),
    linear-gradient(145deg,rgba(255,255,255,.28),transparent 42%,rgba(94,82,70,.08));
  background-size:420px 300px,520px 360px,330px 260px,410px 300px,100% 100%;
}
.tech-stone-header{
  background-color:#d6d2cb;
  background-image:
    radial-gradient(circle at 15% 30%,rgba(255,255,255,.34) 0 2px,transparent 3px),
    radial-gradient(circle at 68% 55%,rgba(80,70,60,.14) 0 1.5px,transparent 2.5px),
    linear-gradient(125deg,transparent 0 32%,rgba(255,255,255,.28) 33%,transparent 36%),
    linear-gradient(25deg,transparent 0 60%,rgba(87,76,65,.10) 61%,transparent 64%);
  background-size:150px 120px,210px 170px,260px 190px,310px 220px;
}
...
.nav{display:flex;gap:.75rem;align-items:center;width:100%;text-align:left;padding:.8rem .75rem;border-radius:.85rem;font-size:.925rem;transition:.15s;color:#475569}
.nav:hover{background:#f1f5f9;color:#0f172a;transform:translateX(2px)}
</style>`;
...
async function hub(){
  ...
  document.getElementById("page").innerHTML=`<main class="max-w-7xl mx-auto p-4 md:p-8 text-[16px] md:text-[17px]">
    <section class="relative overflow-hidden rounded-[2rem] stone-hero border border-slate-300 shadow-sm p-7 md:p-10 mb-8">
      <div class="absolute inset-y-0 right-0 w-1/3 bg-gradient-to-l from-white/45 to-transparent pointer-events-none"></div>
      <div class="absolute -right-16 -top-20 w-56 h-56 rounded-full border-[20px] border-white/35"></div>
      <div class="absolute right-24 top-10 w-2 h-2 rounded-full bg-red-500 shadow-[0_0_14px_rgba(239,68,68,.7)]"></div>
      <div class="relative max-w-4xl">
        <div class="inline-flex items-center gap-2 text-sm font-extrabold tracking-[.20em] text-red-600 bg-red-50 px-4 py-1.5 rounded-full border border-red-200 mb-5"><span class="w-2 h-2 rounded-full bg-red-600"></span> SIAMTAK WORKSPACE</div>
        <h2 class="text-5xl md:text-7xl font-black tracking-[-.04em] text-slate-950 leading-[1.02]">ทำงานอะไรวันนี้?</h2>
        <p class="text-slate-750 mt-4 text-base md:text-lg">เลือกเครื่องมือที่ต้องการใช้งาน หรือค้นหาจากช่องด้านล่าง</p>
        <div class="relative max-w-3xl mt-7">
          <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-xl">⌕</span>
          <input id="workSearch" autocomplete="off" placeholder="ค้นหาเครื่องมือหรือสิ่งที่ต้องการทำ..." class="w-full h-16 pl-12 pr-20 rounded-2xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-red-300 focus:ring-4 focus:ring-red-50 outline-none transition text-sm md:text-base">
    ...
    <section>
      <div class="flex items-center justify-between mb-4"><div class="flex items-center gap-3"><span class="w-1 h-5 rounded-full bg-red-600"></span><h3 class="text-xl md:text-2xl font-black">เครื่องมือ</h3></div><span class="text-[10px] font-bold tracking-[.18em] text-slate-400">${systems.length} TOOLS</span></div>
    ...
    <section class="mt-7 bg-white border border-slate-200 rounded-2xl px-5 py-4 flex gap-4 items-center"><div class="w-9 h-9 rounded-xl bg-slate-100 text-slate-600 grid place-items-center">💬</div><div><h3 class="font-bold text-sm">ติดต่อ Admin</h3><p class="text-xs text-slate-500 mt-0.5">หากต้องการเพิ่มเครื่องมือหรือพบปัญหาในการใช้งาน สามารถติดต่อ Admin</p></div></section>
  </main>${aiBubble()}`;
...
function card(s){return `<button onclick="launch('${esc(s.url)}','${esc(s.type)}')" class="text-left bg-white border border-slate-200 rounded-xl px-4 py-3.5 hover:border-red-200 hover:shadow-sm transition group"><div class="flex items-center gap-3"><div class="w-12 h-12 shrink-0 rounded-lg bg-slate-50 border border-slate-100 group-hover:bg-red-50 group-hover:border-red-100 grid place-items-center text-2xl transition">${esc(s.icon)}</div><div class="min-w-0 flex-1"><h4 class="font-bold text-lg md:text-xl truncate">${esc(s.name)}</h4><p class="text-sm md:text-base text-slate-500 mt-0.5 truncate">${esc(s.description)}</p></div><span class="text-slate-300 group-hover:text-red-500 transition text-lg">↗</span></div></button>`}
